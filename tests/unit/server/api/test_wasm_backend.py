...
class TestWASMAdapterProbeBinary:
    ...
    The probe reports whether the CPython WASM binary is locally resolvable so
    the GraphQL sandboxBackends resolver can surface accurate
    ``SandboxBackendStatus``. It MUST NOT touch the network and MUST NOT
    create cache files; the assertions below pin both rules explicitly.
    """
    ...
    def test_probe_does_not_invoke_real_resolver_network_path(
        ...
        the assertion that the resolver did NOT invoke it is what verifies
        the probe path stays side-effect-free.
